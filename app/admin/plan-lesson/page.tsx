...
function parseTime12ToMinutes(t: string): number | null {
    // Normalize common invisible spaces (NBSP, narrow NBSP) and trim
    const s = (t ?? "")
        .replace(/\u00A0/g, " ")  // NBSP
        .replace(/\u202F/g, " ")  // narrow NBSP
        .replace(/\u2007/g, " ")  // figure space (rare)
        .trim();
...
export default function PlanLessonsPage() {
    ...
    async function createAndGo() {
        ...
        const lessonsPayload = isCancelled
            ? []
            : lessons.map((l) => ({
                time: normalizeNullable(l.time),
                danceId: normalizeNullable(l.danceId), // <-- this is the important missing field
        ...
        const payload = {
            ...
            endDayOffset: eventType?.endDayOffset === 1 ? 1 : 0,

            lessons: lessonsPayload,

            isCancelled: !!isCancelled,
            cancelNote: normalizeNullable(cancellationNote),
